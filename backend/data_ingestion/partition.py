    ...
    def process(self, file_id=None, file=None,topic=None, category_id=None):
        ...
        try:
            ...
            if file_id.endswith('.docx'):
                ...
                raw_pdf_elements = self.pdf_process(file_id)
                CompositeElement = ''
                for element in raw_pdf_elements:
                    if "unstructured.documents.elements.CompositeElement" in str(type(element)):
                        text_elements.append(Document(page_content=str(element),metadata=element.metadata.to_dict().pop('orig_elements')))
                    elif "unstructured.documents.elements.Table" in str(type(element)):
                        element = clean(element.metadata.text_as_html, extra_whitespace=True)
                        table_elements.append(Document(page_content=element,metadata=element.metadata.to_dict().pop('orig_elements')))
